...
\author{Micha\l{} Burdukiewicz\,$^{1}$, Piotr Sobczyk\,$^{2}$, Pawe\l{} 
Mackiewicz\,$^{1}$ and Ma\l{}gorzata Kotulska\,$^{3}$
...
and 
$^{3}$Wroc\l{}aw University of Science and Technology, Department of Biomedical 
...
specific and highly ordered structure of their steric zipper. However, strategies to prevent amyloid formation have been proposed~\citep{hard_inhibition_2012}.
...
relying on general similarities of amino acids, we created multiple encodings 
based on the combinations of various physicochemical properties that 
...
these properties best discriminate between amyloids and non-amyloids.
  ...
  Our model of amyloidogenicity consist of n-gram data sets extracted from 
...
base~\citep{wozniak_amyload:_2015}. Aside from eight sequences shorter than 
five 
...
belonging to more general categories important in the  amyloidogenicity, such 
as size, hydrophobicity, solvent surface area, frequency in $\beta$-sheets and 
contactivity. As a rule of thumb, we limited it to properties introduced after 
1980 when, thanks to the technological advancements, the measurements were more 
accurate. The set of selected physicochemical properties was enriched by six 
measures representing amino acid contact site 
propensities~\cite{wozniak_characteristics_2014} giving in total 26 features.

  Since highly correlated measures create very similar encodings, we further 
reduced 
the number of properties to 17 by selecting measures with the absolute value of 
Pearson's correlation coefficient larger than 0.95 for normalized values 
(Tab.~\ref{tab:properties}). Next, we automatically created 524 284 encodings 
with different 
lengths (from three to six letters) using Ward's 
clusterization~\citep{joe_h._ward_jr_hierarchical_1963} on combinations of 
physicochemical properties. We considered all combinations of 
features with different sizes, so the encodings were created using all 
combinations of $1, 2, \ldots, 16, 17$ features. 
...
\citep{takano_new_2001} \\
  \bottomrule
\end{tabularx}
\caption{Selected 17 physicochemical properties used to create encodings.} 
...
\caption{The scheme of n-gram extraction. A) Input data: peptides with a known 
amyloidogenicity status. B) Extraction of overlapping hexamers. The 
amyloidogenicity status of a source sequence was used as the amyloidogenicity 
status of a derived hexamer. C) Creation of an encoding using a combination of 
physicochemical properties (PP). D) Reduction of the amino acid alphabet 
according to an encoding. E) Extraction of n-grams. From each hexamer we 
extracted countinous n-grams of lengths 1, 2 and 3. In addition, we selected 
gapped 2-grams with the length of gap equal from 1 to 3 residues and gapped 
3-grams with a single gap between the first and the second or the second and 
the third element of the n-gram.}\label{fig:ngram_scheme} 
\end{figure}
...
for 3-grams with a single gap between the first and the second or the second 
and 
...
keeping the total number of positives in both of them fixed. When we impose 
this 
...
independent. We reject null hypothesis, when IG for a feature we test is above 
a 
...
is not an uncommon value, e.g. when one adjusts for multiple testing. Even for 
a 
...
handful of distributions. Therefore complexity of our algorithm is roughly 
equal 
$O(n\cdot p)$.
...
% * <stefan.roediger@b-tu.de> 2016-05-09T18:17:50.376Z:
%
% ^.

\subsection{Cross-validation and selection of the best-performing encoding}

The ability to correctly predict amyloidogenicity was assessed during the five-fold cross-validation. The peptides were assigned randomly to 
...
single hexamer), repeated the cross-validation fifteen times for each 
classifier 
to obtain more precise estimates of performance measures for each classifier. 

  During the testing phase, if at least one hexamer extracted from a peptide 
was 
assessed as amyloidogenic, the whole sequence was denoted as amyloid. 
Otherwise, 
the peptide was classified as non-amyloid. The results were later confronted 
with the known etiquettes of the peptides to compute the performance measures.
...
cross-validation.
  ...
  To choose the most adequate encoding, we ranked the values of Area under 
Curve 
...
the 
lowest sum of ranks from all sequence length categories was selected as the 
best 
...
alphabets, which are very similar to the best-performing encoding, will also have good 
...
where $l$ is equal to the number of physicochemical properties of concern. Hence, 
the nromalizing factor may be interpreted as the minimum change of the mean physicochemical 
properties between groups.

\subsection{Benchmark of AmyGram}

The best-performing encoding chosen during the cross-validation was later used 
to train AmyGram, n-gram based predictor of amyloidogenicity.

  We used \textit{pep424} data set~\citep{walsh_pasta_2014} to compare the 
performance of AmyGram and other predictors of amyloidogenicity. Since the 
model of AmyGram does not cover peptides shorter than five amino acids, we 
...
\caption{Distribution of AUC values of all classifiers based on reduced amino 
acid 
...
amyloidogenicity in the longest peptide data set 16-25, not when learned on 
very 
...
simplest cases of predicting amyloidogenicity, i.e. for the shortest sequences 
(6 
...
shortest sequences available (six residues) were performing the best. That 
might 
...
amyloidogenicity better than the selected encoding. It suggests that despite 
the 
...
analyzed encodings in all categories. It indicates that descriptive divisions 
of 
...
amino acids, while 1 and 5 are mostly hydrophilic. In addition to this, 
subgroup 
1 and 4 have amino acid with the highest average flexibility. The least 
flexible 
...
factor, we used normalized values of described above four features shared by 
all encodings identical to the best-performing one. The value of AUC is 
significantly lower for more distant encodings ($-0.4370$ Pearson's correlation 
coefficient, p-value smaller than $2.2 \times 10^{-16}$). Such relationship 
between AUC and encoding distance indicates that only reduced alphabets 
sufficiently similar to the best-performing encoding are able to remove 
unnecessary diversity while preserving information important for recognition of 
amyloids.
...
have repeats of 3, suggesting that the presence of amino acids belonging to 
this 
...
FoldAmyloid, AmyGram and random forest predictor learned on n-grams extracted 
...
Classifier & AUC & MCC & Sensitivity & Specificity \\ 
  \midrule
AmyGram & \textbf{0.8972} & \textbf{0.6307} & 0.8658 & 0.7889 \\ 
...
The benchmark covered AmyGram as well as three peer-reviewed predictors of 
...
default settings for FoldAmyloid, while PASTA2 evaluated input data in the 
'Peptides' mode.

  Since PASTA2 does not return a probability of belonging to specific category, 
we normalized the output data and used to compute the AUC values. The advised 
...
value 
of specificity 0.9519 is close to value provided by authors (0.95) and assures 
correctness of our computations. In case of other classifiers, we assumed 0.5 
cut-off.
    
  In case of this data set, n-gram extraction method was efficient enough to 
...
encoding of amino acids further increases the general prediction efficiency. 
AmyGram has the highest AUC and MCC among all tested classifiers. Is has 
lower specificity than two classifiers trained on the full alphabet, but still 
outperforms other published method in this category. It is important to 
highlight that AmyGram is the most balanced among all analyzed classifiers, 
having the best Specificity/Sensitivity trade-off, as indicated by the value of 
MCC.

  The specificity of AmyGram is lower than the specificity of PASTA2, which 
is the consequence of the usage of a threshold value optimized for 0.95 
specificity for the latter. If the AmyGram also utilizes such threshold, it 
has much higher sensitivity (0.5518) than PASTA2. Hence, assuming thresholds 
where both predictors detect with the same accurateness true non-amyloids, 
AmyGram predicts more true amyloids. 
 
  Among all considered predictors of amyloidogenicity, APPNN has the highest 
sensitivity. Nevertheless, its AUC is worse than AUCs of all n-gram-based 
