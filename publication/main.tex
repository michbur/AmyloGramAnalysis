...
belonging to more general categories important in the  amyloidogenicity, such as 
size, hydrophobicity, solvent surface area, frequency in $\beta$-sheets and 
...
propensities~\cite{wozniak_characteristics_2014} giving in total 26 features.
  ...
  Since correlated measures create very similar encodings, we further reduced 
...
(Tab.~\ref{tab:properties}). Next, we created 524 284 encodings with different 
lengths (from three to six letters) using Ward's 
clusterization~\citep{joe_h._ward_jr_hierarchical_1963} on combinations of 
physicochemical properties. We considered combinations of all possible sizes 
(1-17).
...
amyloidogenicity status. B) Extraction of everlapping hexamers. The 
...
third element of the n-gram.}\label{fig:ngram_scheme} 
\end{figure}
