...
do_cv <- function(all_folds, extracted_ngrams)
  lapply(extracted_ngrams, function(encoded_group)
    lapply(all_folds, function(fold_list) 
      do_single_cv(fold_list, encoded_group)
    )
  )


do_single_cv <- function(fold_list, encoded_group) {
  lapply(1L:5, function(fold) {
    ...
    preds <- cbind(predict(model_cv, data.frame(rbind(test_pos, test_neg)[, imp_bigrams]))[["predictions"]][, 2], 
