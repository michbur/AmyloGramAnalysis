  ...
  lapply(1L:8, function(dummy) {
    ...
    lapply(1L:5, function(fold) {
      lapply(1L:length(aa_groups), function(group_id) {
        train_pos <- do.call(rbind, coded[[group_id]][fold_list[[1]][fold_list[[1]][, "which"] != fold, "id"]])
        train_neg <- do.call(rbind, coded[[group_id]][fold_list[[3]][fold_list[[3]][, "which"] != fold, "id"]])
        
        test_pos <- rbind(do.call(rbind, coded[[group_id]][fold_list[[1]][fold_list[[1]][, "which"] == fold, "id"]]),
                          do.call(rbind, coded[[group_id]][fold_list[[2]][fold_list[[2]][, "which"] == fold, "id"]]))
        test_neg <- rbind(do.call(rbind, coded[[group_id]][fold_list[[3]][fold_list[[3]][, "which"] == fold, "id"]]), 
                          do.call(rbind, coded[[group_id]][fold_list[[4]][fold_list[[4]][, "which"] == fold, "id"]]))
        ...
        ngram_prots_pos <- c(sapply(coded[[group_id]][fold_list[[1]][fold_list[[1]][, "which"] == fold, "id"]], nrow),
                             sapply(coded[[group_id]][fold_list[[2]][fold_list[[2]][, "which"] == fold, "id"]], nrow))
        ngram_prots_neg <- c(sapply(coded[[group_id]][fold_list[[3]][fold_list[[3]][, "which"] == fold, "id"]], nrow),
                             sapply(coded[[group_id]][fold_list[[4]][fold_list[[4]][, "which"] == fold, "id"]], nrow))
